def test_automation_summary_has_everything_n8n_needs(client):
    v = client.post("/api/varieties", json={"code": "BUM", "name": "Bumble Rumble"}).json()
    p = client.post("/api/plants", json={"origin": "purchased", "variety_id": v["id"]}).json()
    client.post(
        "/api/plantings",
        json={"plant_id": p["id"], "new_location_kind": "container", "new_location_name": "Bak"},
    )
    client.post(
        f"/api/plants/{p['id']}/logs",
        json={"text": "Eerste bloemen", "height_cm": 60, "flower_count": 5, "entry_date": "2026-06-10"},
    )
    client.post(
        f"/api/plants/{p['id']}/logs",
        json={"text": "Geoogst en bemest", "harvested_count": 3, "fertilized": True, "entry_date": "2026-06-14"},
    )
...
def test_automation_hides_gone_plants_by_default(client):
    ...
    p = client.post("/api/plants", json={"origin": "purchased", "variety_id": v["id"]}).json()
    client.post(f"/api/plants/{p['id']}/dispose", json={"kind": "given_away", "recipient": "Jan"})

    assert client.get("/api/automation/plants").json() == []
    assert len(client.get("/api/automation/plants", params={"include_gone": True}).json()) == 1
