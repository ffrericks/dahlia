...
def test_provisional_unknown_plant_has_no_code(client):
    plant = client.post(
        "/api/plants", json={"origin": "unknown", "nickname": "Links"}
    ).json()
...
def test_cannot_split_a_provisional_plant(client):
    ...
    response = client.post(
        "/api/plants", json={"origin": "split", "parent_plant_id": plant["id"]}
    )
    assert response.status_code == 400
...
def test_location_ranking_orders_by_score(client):
    ...
    client.post(
        ...
        json={
            "plant_id": weak["id"],
            "new_location_kind": "garden",
            "new_location_name": "Schaduw",
        },
    )
    client.post(
        f"/api/plants/{good['id']}/logs",
        json={"height_cm": 120, "flower_count": 20, "harvested_count": 15},
    )
    client.post(
        f"/api/plants/{weak['id']}/logs",
        json={"height_cm": 30, "flower_count": 2, "harvested_count": 1},
    )
...
def test_weights_can_exclude_a_metric(client):
    ...
    client.post(
        "/api/plantings", json={"plant_id": plant["id"], "new_location_kind": "garden"}
    )
    ...
    result = client.get(
        ...
        params={"w_height": 0, "w_flowers": 0, "w_harvested": 1},
    ).json()
...
def test_plant_year_history_in_detail(client):
    ...
    client.post(
        f"/api/plants/{plant['id']}/logs", json={"height_cm": 80, "flower_count": 6}
    )
