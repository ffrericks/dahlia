...
def test_update_settings(client):
    updated = client.put(
        "/api/settings",
        json={"tool_url": "http://dahlia.local:8000/", "default_garden_name": "Achtertuin"},
    ).json()
...
def test_default_garden_name_used_for_unnamed_spot(client):
    ...
    loc2 = client.post("/api/locations", json={"kind": "garden", "name": "Zijkant"}).json()
...
def _bak_with_two_plants(client):
    ...
    pa = client.post("/api/plants", json={"origin": "purchased", "variety_id": a["id"]}).json()
    pb = client.post("/api/plants", json={"origin": "purchased", "variety_id": b["id"]}).json()
    loc = client.post("/api/locations", json={"kind": "container", "name": "Bak"}).json()
    client.post("/api/plantings", json={"plant_id": pa["id"], "location_id": loc["id"]})
    client.post("/api/plantings", json={"plant_id": pb["id"], "location_id": loc["id"]})
    return pa, pb
...
def test_auto_fertilize_off_does_not_share(client):
    ...
    client.post(f"/api/plants/{pa['id']}/logs", json={"fertilized": True, "entry_date": "2026-06-15"})

    assert client.get(f"/api/plants/{pa['id']}").json()["last_fertilized"] == "2026-06-15"
...
def test_auto_fertilize_on_shares(client):
    ...
    client.post(f"/api/plants/{pa['id']}/logs", json={"fertilized": True, "entry_date": "2026-06-15"})
    assert client.get(f"/api/plants/{pb['id']}").json()["last_fertilized"] == "2026-06-15"
