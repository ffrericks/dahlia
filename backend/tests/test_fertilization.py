...
def test_fertilize_only_entry_is_allowed(client):
    ...
    p = client.post("/api/plants", json={"origin": "purchased", "variety_id": v["id"]}).json()
    resp = client.post(
        f"/api/plants/{p['id']}/logs", json={"fertilized": True, "entry_date": "2026-06-10"}
    )
...
def test_last_fertilized_shown_on_plant(client):
    v = _variety(client, "WIT")
    p = client.post("/api/plants", json={"origin": "purchased", "variety_id": v["id"]}).json()
    client.post(f"/api/plants/{p['id']}/logs", json={"fertilized": True, "entry_date": "2026-06-01"})
    client.post(f"/api/plants/{p['id']}/logs", json={"fertilized": True, "entry_date": "2026-06-20"})
...
def test_fertilizing_one_plant_counts_for_others_in_the_same_bak(client):
    ...
    client.post(f"/api/plants/{a['id']}/logs", json={"fertilized": True, "entry_date": "2026-06-15"})

    # Plant B, sharing the bak, shows the same last-fertilized date.
    assert client.get(f"/api/plants/{b['id']}").json()["last_fertilized"] == "2026-06-15"
...
def test_plant_in_another_spot_is_not_affected(client):
    ...
    assert client.get(f"/api/plants/{a['id']}").json()["last_fertilized"] == "2026-06-15"
...
def test_same_date_in_one_bak_is_one_feeding(client):
    ...
    client.post(f"/api/plants/{a['id']}/logs", json={"fertilized": True, "entry_date": "2026-06-15"})
    client.post(f"/api/plants/{b['id']}/logs", json={"fertilized": True, "entry_date": "2026-06-15"})
