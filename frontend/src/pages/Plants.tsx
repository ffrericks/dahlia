...
import { createPlant, getSummary, listPlants } from '../api/plants'
    ...
    <div className="flex flex-col gap-4">
      ...
      {!loading && plants.length === 0 && (
        <p className="rounded-xl border border-dashed border-stone-300 p-8 text-center text-stone-500">
          Nog geen planten. Voeg je eerste plant toe (gekocht, gekregen, afsplitsing of zaailing).
        </p>
      )}
      ...
      <ul className="flex flex-col gap-2">
        {plants.map((plant) => (
          <li key={plant.id}>
            <button
              onClick={() => setView({ mode: 'detail', id: plant.id })}
              className={`flex w-full items-center gap-3 rounded-xl border border-stone-200 bg-white p-3 text-left shadow-sm hover:bg-stone-50 ${
                plant.state === 'discarded' || plant.state === 'given_away' ? 'opacity-50' : ''
              }`}
            >
              {plant.thumbnail ? (
                <img
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-sm text-stone-500">
                  ...
                  {plant.location && (
                    <span className="font-mono text-stone-400"> · {plant.location.code}</span>
                  )}
                </p>
              </div>
...
function Badge({ label }: { label: string }) {
  return (
    <span className="rounded-full bg-stone-100 px-3 py-1 text-stone-600">{label}</span>
  )
}
